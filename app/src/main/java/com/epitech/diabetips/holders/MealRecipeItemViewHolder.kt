...
class MealRecipeItemViewHolder(inflater: LayoutInflater, parent: ViewGroup)
    ...
    fun bind(recipe: MealRecipeObject, onItemClickListener : ((MealRecipeObject) -> Unit)? = null) {
        ...
        mealRecipeQuantity?.text = "${recipe.portions_eaten.toBigDecimal().stripTrailingZeros().toPlainString()} ${context.getString(R.string.unit_portions)} (${recipe.getQuantity().toBigDecimal().stripTrailingZeros().toPlainString()} ${context.getString(R.string.unit_g)})"
